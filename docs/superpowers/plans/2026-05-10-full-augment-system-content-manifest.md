...
Every augment below must become a `data/content/augments/<route>/<id>.tres` `AugmentData` resource and must be covered by a route smoke test or a specific content contract.
...
Each item must preserve these source fields:

```text
id, display_name, source_augment_name, source_augment_rarity, route_id, route_label, rarity, max_rank, unique, upgrade_type, trigger, effect, condition, value, synergy_tags, required_tags, excludes_tags, combo_value, fit, risk, why_close, implementation_hint
```
